...
function controlId(parts: string[]) {
  return parts.map((part) => encodeURIComponent(part)).join(":");
}

function reason(at: string, code: AcquisitionJob["lastExplanation"]["reason"], detail?: string) {
  return { at, reason: code, ...(detail ? { detail } : {}) };
}
...
class FirestoreAcquisitionRuntimeStore implements AcquisitionRuntimeStore {
  ...
  async listDueJobs(input: { beforeOrAt: string; limit: number; dataMode?: AnalyticsDataMode }) {
    ...
    return snap.docs.map((doc) => doc.data() as AcquisitionJob);
  }
  ...
  async tryLeaseJob(input: { jobId: string; workerId: string; leaseToken: string; leasedAt: string; leaseExpiresAt: string }) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      if (job.providerSubmissionStartedAt) {
        const updated: AcquisitionJob = { ...job, status: "unknown-outcome", lease: undefined, updatedAt: input.leasedAt, lastExplanation: reason(input.leasedAt, "provider-unknown-outcome", "Expired lease crossed the provider ambiguity barrier.") };
        tx.set(ref, updated, { merge: false });
        return { status: "unknown-outcome" as const, job: updated };
      }
      const leased: AcquisitionJob = {
        ...job,
        status: "leased",
        lease: { leaseToken: input.leaseToken, workerId: input.workerId, leasedAt: input.leasedAt, expiresAt: input.leaseExpiresAt },
        updatedAt: input.leasedAt,
        lastExplanation: reason(input.leasedAt, "scheduled"),
      };
  ...
  async markProviderSubmissionStarted(input: Parameters<AcquisitionRuntimeStore["markProviderSubmissionStarted"]>[0]) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      const frequencyQuery = db.collection(JOBS)
      ...
      const reservations = await tx.get(frequencyQuery);
      if (reservations.size >= input.frequencyAdmission.maxProviderAcceptedEffects) {
        const suppressed: AcquisitionJob = {
          ...job,
          status: "suppressed",
          lease: undefined,
          updatedAt: input.at,
          lastExplanation: reason(input.at, "frequency-cap-reached"),
        };
      ...
      const marked = {
        ...
        providerPurpose: input.frequencyAdmission.purpose,
  ...
  async transitionLeasedJob(input: Parameters<AcquisitionRuntimeStore["transitionLeasedJob"]>[0]) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      const next: Record<string, unknown> = {
        ...
        lease: null,
        updatedAt: input.at,
        lastExplanation: reason(input.at, input.reason, input.detail),
      };
      ...
      if (input.clearProviderSubmissionMarker) {
        next.providerSubmissionStartedAt = null;
        next.providerSubmissionAttemptId = null;
        next.providerPurpose = null;
      }
      tx.set(ref, next, { merge: false });
      const saved = { ...current, ...next, lease: undefined } as unknown as AcquisitionJob;
      return saved;
    });
  }
  ...
  async getPauseState(input: { organizationId: string; automationId: AcquisitionCatalogId; dataMode: AnalyticsDataMode }) {
    ...
    return {
      // Fail closed: a missing explicit platform activation record means paused.
  ...
  async countProviderAcceptedEffects(input: { organizationId: string; subjectId: string; dataMode: AnalyticsDataMode; purpose: string; since: string }) {
    const snap = await db.collection(JOBS)
      ...
      .where("providerPurpose", "==", input.purpose)
  ...
  async cancelPending(input: { organizationId: string; subjectId: string; dataMode: AnalyticsDataMode; automationId?: AcquisitionCatalogId; at: string; reason: AcquisitionJob["lastExplanation"]["reason"]; detail?: string }) {
    let query = db.collection(JOBS)
      ...
      .where("subjectId", "==", input.subjectId)
      .where("dataMode", "==", input.dataMode)
      .where("status", "in", ["scheduled", "held", "retrying", "leased"]);
    if (input.automationId) query = query.where("automationId", "==", input.automationId);
    const snap = await query.limit(200).get();
    const batch = db.batch();
    for (const doc of snap.docs) batch.update(doc.ref, { status: "cancelled", lease: null, updatedAt: input.at, lastExplanation: reason(input.at, input.reason, input.detail) });
    if (!snap.empty) await batch.commit();
    return snap.size;
  }

  async finalizeEnrollmentIfSettled(input: { enrollmentId: string; at: string }) {
    const enrollmentRef = db.collection(ENROLLMENTS).doc(input.enrollmentId);
    const [enrollment, jobs] = await Promise.all([
      enrollmentRef.get(),
      db.collection(JOBS).where("enrollmentId", "==", input.enrollmentId).get(),
    ]);
    ...
    const status = statuses.some((value) => value === "failed" || value === "unknown-outcome") ? "failed" : statuses.every((value) => value === "cancelled") ? "cancelled" : "completed";
    ...
    await enrollmentRef.set(updated, { merge: false });
    return updated;
  }

  async getOperationsSnapshot(input: { organizationId?: string; dataMode?: AnalyticsDataMode; limit: number }) {
    let query = db.collection(JOBS).orderBy("updatedAt", "desc").limit(Math.min(Math.max(input.limit, 1), 100));
    const snap = await query.get();
    const recentJobs = snap.docs.map((doc) => doc.data() as AcquisitionJob)
      .filter((job) => !input.organizationId || job.organizationId === input.organizationId)
      .filter((job) => !input.dataMode || job.dataMode === input.dataMode);
    ...
    return {
      ...
      counts,
      recentJobs,
      backendPersistence: "ready" as const,
      scheduler: "ready" as const,
      note: platform.data()?.paused === false ? "Runtime is explicitly enabled." : "Runtime is deployed fail-closed; outbound acquisition remains paused.",
    };
  }
}
...
const definitions: AcquisitionDefinitionPort = {
  async listPublishedForTrigger(input) {
    const snap = await db.collection(DEFINITIONS)
      .where("organizationId", "==", input.organizationId)
      .where("triggerEventType", "==", input.eventType)
      .where("enabled", "==", true)
      .get();
...
const state: AcquisitionStatePort = {
  async readCurrentState(input) {
    ...
    const subjectRef = input.subjectKind === "customer"
      ? db.collection("organizations").doc(input.organizationId).collection("customers").doc(input.subjectId)
      : db.collection("organizations").doc(input.organizationId).collection("leads").doc(input.subjectId);
    const subject = await subjectRef.get();
    const projection = input.customerId
      ? await db.collection("lifecycleProjections").doc(hash(`${input.organizationId}:${input.customerId}:${input.dataMode}`)).get()
      : null;
...
// Release 2 backend can be deployed before provider activation. This port fails
// closed until D sender/secrets/consent/provider proof is explicitly completed.
const email: AcquisitionEmailDispatchPort = {
  async evaluate() {
    return { status: "hold", checkedAt: new Date().toISOString(), reason: "Outbound acquisition email is not activated.", code: "sender-not-ready" };
  },
  async submit() {
    return { status: "suppressed", reason: "Outbound acquisition email is not activated." };
  },
};
...
export const drainAcquisitionJobs = onSchedule({ schedule: "every 5 minutes", timeZone: "Etc/UTC" }, async () => {
  const platform = await db.collection(CONTROLS).doc("platform").get();
  // Missing/paused is a no-op rather than cancelling queued jobs. Explicit
  // provider activation must first set paused=false through an audited admin path.
