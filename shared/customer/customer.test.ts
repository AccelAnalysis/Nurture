...
import {
  ...
  migrateLegacyIdentityOnboarding,
  ...
  steps: [
    {
      ...
      description: "Profile",
      required: true,
      questions: [{ id: "displayName", label: "Display name", type: "text", required: true, purpose: "Address the customer." }],
    },
    {
      id: "terms",
      route: "terms",
      ...
      required: true,
      questions: [],
      agreement: { id: "terms", version: "2026-09", label: "Terms", required: true },
    },
  ],
};
...
describe("Release 2 Track C customer foundation", () => {
  it("preserves the stable Release 1 customer ID while tenant progress stays independent", () => {
    ...
    expect(onboardingProgressId(scope("org-a"))).toBe(onboardingProgressId(scope("org-b")));
    // Progress IDs are document-local inside an organization; the full tenant path supplies isolation.
  ...
  it("keeps missing consent unknown rather than permission", () => {
    expect(consentState(null)).toBe("unknown");
  });

  it("pins an in-progress flow and emits logical completion only once", () => {
    const started = createOnboardingProgress(scope("org-a"), definition, "2026-09-05T12:00:00.000Z");
    expect(started.flowVersion).toBe("2.0.0");
  ...
  it("resumption clears an abandonment inference without changing the pinned version", () => {
    const started = createOnboardingProgress(scope("org-a"), definition, "2026-09-01T12:00:00.000Z");
    const abandoned = { ...started, status: "abandoned" as const, abandonedAt: "2026-09-05T12:00:00.000Z" };
    const resumed = resumeOnboardingProgress(abandoned, "2026-09-05T13:00:00.000Z");
    expect(resumed.status).toBe("in-progress");
    expect(resumed.abandonedAt).toBeUndefined();
    expect(resumed.flowVersion).toBe("2.0.0");
  });

  it("migrates legacy onboarding only into an explicitly selected tenant scope", () => {
    const legacy: LegacyIdentityOnboardingState = {
      identityId: "uid-1",
      customerId: "customer_uid-1",
      definitionId: "nurture.release-1.default",
      definitionVersion: "1.0.0",
      status: "complete",
      steps: { profile: "complete", terms: "complete" },
      answers: { displayName: "Casey" },
      acceptedAgreements: { terms: { agreementId: "terms", version: "2026-09", acceptedAt: "2026-08-01T12:00:00.000Z" } },
      startedAt: "2026-08-01T12:00:00.000Z",
      lastActivityAt: "2026-08-01T12:10:00.000Z",
