...
export function BrandSiteAdminPage({ organizationId }: { organizationId: string }) {
  ...
  const resetDraft = () => {
    ...
    setDraft(createNurtureDefaultConfiguration(organizationId));
    setMessage("Draft reset to Nurture defaults. The published public site has not changed.");
  };
  ...
  return (
    <div className="track-a-admin-page">
      <PageHeader
        ...
        actions={<div className="track-a-publish-actions"><Button className="button-secondary" onClick={saveDraft}>Save draft</Button><Button onClick={publish} disabled={Boolean(errors.length)}>Publish</Button></div>}
      />

      <div className="track-a-status-row" aria-live="polite">
        <Badge tone={Object.keys(overrides).length ? "warning" : "neutral"}>{Object.keys(overrides).length ? "Draft has overrides" : "Using Nurture defaults"}</Badge>
      ...
      <Card className="track-a-launch-checklist">
        ...
        <ChecklistItem title="Offers" detail="Commercial configuration is owned by Track D and consumes the same organization boundary." status="Track D" tone="accent" />
        <ChecklistItem title="Experience" detail="Public/trial module capability is owned by Track B; Track A only provides the CTA handoff." status="Track B" tone="accent" />
        <ChecklistItem title="Onboarding" detail="Registration and onboarding are owned by Track C; Track A preserves their public route handoffs." status="Track C" tone="accent" />
        <ChecklistItem title="Governance" detail="Production persistence, tenant authorization, and audit events plug into the Track E service boundary." status="Track E" tone="accent" />
      </Card>
      ...
      <div className="track-a-workspace">
        <div className="track-a-editor">
          ...
          <div className="track-a-destructive-row"><button type="button" onClick={resetDraft}>Reset entire draft to Nurture defaults</button><span>Published configuration is unaffected until you publish again.</span></div>
        </div>
        ...
        <aside className="track-a-preview-panel">
          ...
          <p className="muted">This preview renders the effective draft only. Public routes continue to use the active published configuration until Publish is selected.</p>
