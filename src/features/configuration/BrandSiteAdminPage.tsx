...
export function BrandSiteAdminPage({ organizationId }: { organizationId: string }) {
  ...
  const canManage = localDemoEnabled && access.can("brand.manage");
  const canPublish = localDemoEnabled && access.can("brand.publish");
  ...
  return (
    <div className="track-a-admin-page">
      {!localDemoEnabled ? <Card><p role="status">Server-backed configuration publishing is not available. This screen cannot publish browser-only changes to the public application.</p></Card> : <Card><Badge tone="warning">Local preview only</Badge><p>Drafts and publications in this development preview exist in this browser only.</p></Card>}
      <PageHeader
