...
import { firebaseConfigured as firebaseClientConfigured } from "../../firebase";
const firebaseConfigured = firebaseClientConfigured && releaseBackendReady;
...
function fallbackPublishedOffers(organizationId?: string) {
  ...
  return firebaseConfigured ? [] : releaseOneDefaultOffers.map((offer) => ({ ...offer, organizationId: organizationId ?? offer.organizationId }));
}
...
export function PublicOffersPage({ organizationId = DEMO_ORG_ID }: { organizationId?: string }) {
  const [offers, setOffers] = useState<CommercialOffer[]>(() => fallbackPublishedOffers(organizationId));
      ...
      <div className="pricing-grid">
        {offers.filter((offer) => !["disabled", "archived"].includes(offer.status)).sort((a, b) => a.order - b.order).map((offer) => (
          <Card key={offer.id}>
            ...
            <PriceChoice offer={offer} interval={interval} onInterval={setInterval} />
            <OfferBenefits offer={offer} />
            <Button disabled={!firebaseConfigured || busyOfferId === offer.id} onClick={() => void beginCheckout(offer)}>{busyOfferId === offer.id ? "Opening checkout…" : "Continue"}</Button>
          </Card>
        ))}
      </div>
    </>
...
export function ParticipantBillingPage() {
  ...
  return (
    <>
      ...
      ) : <EmptyState title="No verified subscription" description={firebaseConfigured ? "No provider-backed subscription is currently recorded for this organization/customer relationship." : "Purchases are unavailable while this application is being prepared."} action={<Link className="button" href="/app/offers">View offers</Link>} />}
    </>
  );
}
