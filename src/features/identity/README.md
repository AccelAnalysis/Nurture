...
- A distinct `CustomerProfile` bootstrap. A Firebase `User` is authentication identity; the customer profile is Nurture account-domain state; `OrganizationMembership` and an organization-scoped Customer mapping remain separate concepts.
...
Browser lifecycle signals are observations, not trusted persisted lifecycle events. They use `eventId`, correlation/idempotency IDs, identity/customer hints, `transport: "browser"`, and `trust: "client-observed"` so Track F can normalize them without confusing browser transport with the persisted trusted `source`. Track F/the lifecycle ingestion boundary must bind verified organization/customer context and server receipt metadata before persistence. Candidate organization IDs captured during acquisition are deliberately named as candidates and must not confer tenant authority.

Track E must own the Firestore security rules and server-side authorization for the identity collections described in `docs/track-c-identity-onboarding.md`. `customerScopeSource` also fails closed for organization-scoped resolution until the trusted organization-Customer bootstrap/link boundary is composed with Tracks E/D/B. Track C does not create organization memberships, entitlements, or platform roles.
