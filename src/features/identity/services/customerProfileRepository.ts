...
import { stableCustomerIdForIdentity } from "../../../../shared/customer/identity";
...
export const defaultCustomerPreferences: UserPreferences = { theme: "system", emailNotifications: true, smsNotifications: false, pushNotifications: true };
export function customerIdForIdentity(identityId: string) { return stableCustomerIdForIdentity(identityId); }
function buildProfile(user: User): CustomerProfile {
  const now = new Date().toISOString();
  return { customerId: customerIdForIdentity(user.uid), identityId: user.uid, email: user.email, displayName: user.displayName, firstName: null, lastName: null, phone: user.phoneNumber, status: "active", onboardingStatus: "not-started", preferences: defaultCustomerPreferences, createdAt: user.metadata.creationTime ?? now, updatedAt: now };
}
export const customerProfileRepository = {
  get(identityId: string) { return identityDocumentStore.read<CustomerProfile>(identityCollections.customers, identityId); },
  ...
  async update(identityId: string, changes: CustomerProfileChanges): Promise<CustomerProfile> {
    ...
    const next: CustomerProfile = { ...current, ...changes, customerId: current.customerId, identityId: current.identityId, preferences: changes.preferences ? { ...current.preferences, ...changes.preferences } : current.preferences, updatedAt: new Date().toISOString() };
