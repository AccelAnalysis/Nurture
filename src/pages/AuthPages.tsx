...
export function AuthPage({ mode }: { mode: "login" | "register" | "forgot" | "verify" }) {
  ...
  const returnTo = route.query.get("returnTo") || "/app";
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (mode === "forgot") {
        ...
        setMessage("Password reset instructions requested. Check your email if this account exists.");
        return;
      }
      navigate(returnTo);
  ...
  if (mode === "verify") {
    return (
      <AuthFrame>
        <PageHeader eyebrow="Account" title="Verify your email" description="Email verification is represented as a dedicated state so onboarding can pause here when required." />
        <Card>
          <p>When Firebase email verification is enabled for an account, this route will confirm status and offer resend actions.</p>
          <Button onClick={() => authService.sendVerification().then(() => setMessage("Verification email requested.")).catch((error: Error) => setMessage(error.message))}>Resend verification</Button>
  ...
  const description = mode === "login"
    ...
    : mode === "register"
      ? "Create an individual account now; organization creation and invitations attach through membership records."
    ...
    <AuthFrame>
      ...
      <Card className="form-card">
        <form onSubmit={submit}>
          ...
          {mode !== "forgot" ? <label>Password<Input required type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} /></label> : null}
          <Button disabled={busy}>{busy ? "Working…" : mode === "login" ? "Sign in" : mode === "register" ? "Create account" : "Send reset link"}</Button>
        </form>
        {message ? <p className="form-message">{message}</p> : null}
        {mode === "login" ? (
          <>
            <div className="divider"><span>or review the skeleton</span></div>
