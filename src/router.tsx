...
export function Link({ href, children, className, ...props }: AnchorHTMLAttributes<HTMLAnchorElement> & { href: string; children: ReactNode }) {
  const handleClick = (event: MouseEvent<HTMLAnchorElement>) => {
    props.onClick?.(event);
    if (event.defaultPrevented || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || props.target === "_blank" || href.startsWith("http") || href.startsWith("mailto:") || href.startsWith("tel:")) return;
...
export function Protected({ children }: { children: ReactNode }) {
  ...
  if (!currentUser) {
    queueMicrotask(() => navigate(`/login?returnTo=${encodeURIComponent(window.location.pathname)}`, true));
    return <LoadingState label="Preparing sign in…" />;
  }
  return <>{children}</>;
}

export function OrganizationProtected({ children }: { children: ReactNode }) {
  const { currentUser, loading } = useAuth();
  ...
  if (!canManage) return <div className="content-width"><div className="state-panel error-state">Your organization role does not include administration access.</div></div>;
  return <>{children}</>;
}
